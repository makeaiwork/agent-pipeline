...
| 7   | Hooks: `safety-check` (Bash), `agent-sync-check` (Agent), `post-edit`, `pre-compact`, `session-start`, `notify`; registered in `settings.json`; tests green       | `jq .hooks`, a run of `*.test.sh`                                                 |
...
| 12  | `.claude/state`, `.claude/worktrees` in `.gitignore`; `.worktreeinclude`; Dropbox xattr                                                                           | `git check-ignore`, `xattr -p`                                                    |
